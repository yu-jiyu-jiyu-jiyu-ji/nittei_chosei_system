...
""",
    ...
    # 分割検索（F）: 週を1日ずつ検索し、1リクエストあたりの処理時間を短くする
    cjob = st.session_state.get("candidate_search_job")
    if cjob is not None:
        step = int(cjob.get("step", 0))
        if step < 7:
            ...
            with st.expander("前現場の住所がカレンダーにない予定（暫定住所）", expanded=False):
                ...
                for eid, rows in by_eid.items():
                    ...
                    if len(rows) > 1:
                        ...
                        st.write(f"**{names}** さんの前予定「{summ}」に住所がありません。")
                        ...
                        r = rows[0]
                        st.write(
                            f"**{r.get('worker_name', '')}** さんの前予定「{summ}」に住所がありません。"
                        )
    ...
    try:
        # 検索ボタン／週ナビ再検索 → 分割ジョブを開始（各日は candidate_search_job ブロックで実行）
    ...
    if dcid and filtered:
        ...
        if target is None:
            ...
            def _show_candidate_detail() -> None:
                ...
                with col_close:
                    if st.button("閉じる", key=f"dialog_close_{dcid}"):
                        st.session_state.pop("candidate_dialog_id", None)
                        st.session_state.pop(PLOTLY_CALENDAR_KEY, None)
                        st.rerun()
                with col_decide:
                    if st.button("決定", type="primary", key=f"dialog_decide_{dcid}"):
                        ...
                        try:
                            ...
                            st.exception(exc)
                            return
                        with st.expander("登録結果", expanded=True):
                            for m in msgs:
                                st.write(m)
